...
local function GetURL(scripturl)
	if shared.goatDeveloper then
		assert(betterisfile("goat/"..scripturl), "File not found : goat/"..scripturl)
		return readfile("goat/"..scripturl)
	else
		local res = game:HttpGet("https://raw.githubusercontent.com/cinarkagan/GoatV4/main/"..scripturl, true)
		assert(res ~= "404: Not Found", "File not found : goat/"..scripturl)
		return res
	end
end
...
if shared.goatWaiting then repeat task.wait() until shared.RequireTable end
if shared.RequireTable == nil then 
	if shared.RequireTable == nil then
		if queueteleport then
            ...
            prompt:_open("Your exploit is unsupported by Phantom Forces goat.")
            task.wait(9e9)
		end
	end
end
...
local teleportfunc = game:GetService("Players").LocalPlayer.OnTeleport:Connect(function(State)
	if (not teleported) and (not shared.goatIndependent) then
		teleported = true
		local finaltp = (tpstring and 'shared.goatoverlay = "'..tpstring..'"\n' or "").."shared.scriptalreadyjoinedservers = '"..httpservice:JSONEncode(alreadyjoined).."'\n"..bypassScript
