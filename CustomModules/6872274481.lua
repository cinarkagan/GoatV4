...
local targetinfo = shared.goatTargetInfo
...
local WhitelistFunctions = shared.goatwhitelist
...
local function getcustomassetfunc(path)
	if not betterisfile(path) then
		...
		local req = requestfunc({
			Url = "https://raw.githubusercontent.com/cinarkagan/GoatV4/main/"..path:gsub("goat/assets", "assets"),
			Method = "GET"
		})
...
local function CreateAutoHotbarGUI(children2, argstable)
	...
	addbutton.Image = getcustomassetfunc("goat/assets/AddItem.png")
	...
	ItemListExitButton.Image = getcustomassetfunc("goat/assets/ExitIcon1.png")
	...
	ItemListFrameShadow.Image = getcustomassetfunc("goat/assets/WindowBlur.png")
...
runcode(function()
    getfunctions = function()
		...
		if not shared.goatbypassed then
			...
			shared.goatbypassed = true
		end

		task.spawn(function()
			local chatsuc, chatres = pcall(function() return game:GetService("HttpService"):JSONDecode(readfile("goat/Profiles/bedwarssettings.json")) end)
			if chatsuc then
				if chatres.crashed and (not chatres.said) then
					pcall(function()
						createwarning("goat", "either ur poor or its a exploit moment", 10)
						createwarning("goat", "getconnections crashed, chat hook not loaded.", 10)
					end)
					local jsondata = game:GetService("HttpService"):JSONEncode({
						crashed = true,
						said = true,
					})
					writefile("goat/Profiles/bedwarssettings.json", jsondata)
				end
											...
											MessageData.ExtraData = {
												...
												Tags = {
													table.unpack(MessageData.ExtraData.Tags),
													{
														...
														TagText = "goat PRIVATE"
													}
												}
											}
										end
										if plrtype == "goat OWNER" then
											MessageData.ExtraData = {
...
connectionstodisconnect[#connectionstodisconnect + 1] = lplr.OnTeleport:Connect(function(State)
	if (not teleportedServers) then
		...
		if clientstorestate.Party and clientstorestate.Party.members and #clientstorestate.Party.members > 0 then
        	queuedstring = queuedstring..'shared.goatteammembers = '..#clientstorestate.Party.members..'\n'
		end
		if tpstring then
			queuedstring = queuedstring..'shared.goatoverlay = "'..tpstring..'"\n'
		end
...
local function getNametagString(plr)
	...
	if WhitelistFunctions:CheckPlayerType(plr) == "goat PRIVATE" then
		nametag = '<font color="rgb(127, 0, 255)">[goat PRIVATE] '..(plr.Name)..'</font>'
	end
	if WhitelistFunctions:CheckPlayerType(plr) == "goat OWNER" then
		nametag = '<font color="rgb(255, 80, 80)">[goat OWNER] '..(plr.DisplayName or plr.Name)..'</font>'
	end
		...
		if (WhitelistFunctions:CheckPlayerType(plr) ~= "DEFAULT" or WhitelistFunctions.WhitelistTable.chattags[WhitelistFunctions:Hash(plr.Name..plr.UserId)]) then
			...
			if playerlist then
				pcall(function()
					...
					if targetedplr then 
						targetedplr.ChildrenFrame.NameFrame.BGFrame.OverlayFrame.PlayerIcon.Image = getcustomassetfunc("goat/assets/goatIcon.png")
					end
				end)
			end
			if lplr ~= plr and WhitelistFunctions:CheckPlayerType(lplr) == "DEFAULT" then
				task.spawn(function()
					...
					task.wait(4)
					repstorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer("/w "..plr.Name.." "..clients.ChatStrings2.goat, "All")
					task.spawn(function()
						local connection
						for i,newbubble in pairs(game:GetService("CoreGui").BubbleChat:GetDescendants()) do
							if newbubble:IsA("TextLabel") and newbubble.Text:find(clients.ChatStrings2.goat) then
				...
				if char then
					task.spawn(function()
						pcall(function() 
							...
							Cape(char, getcustomassetfunc("goat/assets/goatCape.png"))
						end)
					end)
				end
			end
...
runcode(function()
	...
	targetinfo["UpdateInfo"] = function(tab, targetsize)
		...
		pcall(function()
			for i,v in pairs(shared.goatTargetInfo.Targets) do
...
runcode(function()
	...
	local killauranogoat = {Enabled = false}
	...
	local function newAttackEntity(plr, firstplayercodedone, attackedplayers)
		...
		if killauranogoat.Enabled and clients.ClientUsers[plr.Player.Name] then
			return nil
		end
	...
	if WhitelistFunctions:CheckPlayerType(lplr) ~= "DEFAULT" then
		killauranogoat = Killaura.CreateToggle({
			Name = "No goat",
			Function = function() end,
			HoverText = "no hit goat user"
		})
	end
end)
...
local function findplayers(arg, plr)
	...
	if arg == "private" and continuechecking and WhitelistFunctions:CheckPlayerType(lplr) == "goat PRIVATE" then table.insert(temp, lplr) continuechecking = false end
		...
		game:GetService("CoreGui"):FindFirstChild("RobloxPromptGui"):FindFirstChild("promptOverlay").DescendantAdded:Connect(function(obj)
			...
			if obj.Name == "LeaveButton" then
				...
				clone.MouseButton1Click:Connect(function()
					...
					video.Video = getcustomassetfunc("goat/assets/skill.webm")
...
runcode(function()
	...
	connectionstodisconnect[#connectionstodisconnect + 1] = repstorage.DefaultChatSystemChatEvents.OnMessageDoneFiltering.OnClientEvent:Connect(function(tab, channel)
		...
		if plr and WhitelistFunctions:CheckPlayerType(lplr) ~= "DEFAULT" and tab.MessageType == "Whisper" and client ~= nil and alreadysaidlist[plr.Name] == nil then
			...
			createwarning("goat", plr.Name.." is using "..client.."!", 60)
...
runcode(function()
	...
	local function autoleaveplr(plr)
		task.spawn(function()
			...
			if getRole(plr) >= 100 and (plr.UserId ~= 87365146 or shared.goatPrivate) then
				if AutoLeaveStaff.Enabled then
					if AutoLeaveStaff2.Enabled then 
						createwarning("goat", "Staff Detected : "..(plr.DisplayName and plr.DisplayName.." ("..plr.Name..")" or plr.Name).." : Play legit like nothing happened to have the highest chance of not getting banned.", 60)
					...
					createwarning("goat", "Staff Detected : "..(plr.DisplayName and plr.DisplayName.." ("..plr.Name..")" or plr.Name), 60)
				end
			end
		end)
	end
	...
	AutoLeaveStaff2 = AutoLeave.CreateToggle({
		...
		Function = function() end,
		HoverText = "Instead of uninjecting, It will now reconfig goat temporarily to a more legit config.",
		Default = true
	})
...
runcode(function()
	...
	local function finishcframe(cframe)
		return shared.goatOverrideAnticheatBypassCFrame and shared.goatOverrideAnticheatBypassCFrame(cframe) or cframe
	end
	...
	local function disablestuff()
		...
		shared.goatRealCharacter = {
			Humanoid = entityLibrary.character.Humanoid,
			Head = entityLibrary.character.Head,
			HumanoidRootPart = oldcloneroot
		}
		if shared.goatOverrideAnticheatBypassPre then 
			shared.goatOverrideAnticheatBypassPre(lplr.Character)
		end
		repeat
			task.wait()
			if entityLibrary.isAlive then
				...
				if oldroot then
					...
					if cloneroot then
						if oldroot.Parent ~= nil and (not isnetworkowner(oldroot)) then
							...
							if not shared.goatOverrideAnticheatBypass then
		...
		Function = function(callback)
			if callback then
				...
				if #customlink > 0 and capebox.Value:len() > 3 and capebox.Value:sub(capebox.Value:len() - 3, capebox.Value:len()):lower() == ".png" then
					if (not betterisfile("goat/assets/"..customlink[#customlink])) then 
						local suc, res = pcall(function() writefile("goat/assets/"..customlink[#customlink], game:HttpGet(capebox.Value, true)) end)
						if not suc then 
				...
				goatcapeconnection = lplr.CharacterAdded:Connect(function(char)
					task.spawn(function()
						pcall(function() 
							Cape(char, getcustomassetfunc("goat/assets/"..(successfulcustom and customlink[#customlink] or "goatCape.png")))
						end)
					end)
				end)
				if lplr.Character then
					task.spawn(function()
						pcall(function() 
							Cape(lplr.Character, getcustomassetfunc("goat/assets/"..(successfulcustom and customlink[#customlink] or "goatCape.png")))
						end)
					end)
				end
			else
				if goatcapeconnection then
					goatcapeconnection:Disconnect()
				end
				if lplr.Character then
					for i,v in pairs(lplr.Character:GetDescendants()) do
						if v.Name == "Cape" then
...
runcode(function()
	tpstring = shared.goatoverlay or nil
				...
				if oldnetworkowner ~= nil and oldnetworkowner ~= newnetworkowner and newnetworkowner == false and notlasso() then 
					...
					if (not teleported[lplr]) and respawnflag then
						task.delay(1, function()
							...
							if falseflag then 
								createwarning("goat", "Lagspike Detected : "..falseflag, 10)
							else
								lagbacks = lagbacks + 1
							end
						end)
						...
						if respawnflag then
							createwarning("goat", "Telepearl False Flag", 10)
						end
					end
				end
...
task.spawn(function()
	local url = "https://raw.githubusercontent.com/cinarkagan/GoatV4/main/CustomModules/bedwarsdata"
	...
	local function createannouncement(announcetab)
		...
		notifyframelistnotifyframeaspect.Text = "goat Announcement"
	...
	local function rundata(datatab, olddatatab)
		if not olddatatab then
			if datatab.Disabled then 
				...
				game:GetService("StarterGui"):SetCore("SendNotification", {
					Title = "goat",
					Text = "goat is currently disabled, check the discord for updates discord.gg/goatroblox",
					Duration = 30,
				})
			end
	...
	task.spawn(function()
		pcall(function()
			if betterisfile("goat/Profiles/bedwarsdata.txt") == false then 
				writefile("goat/Profiles/bedwarsdata.txt", game:HttpGet(url, true))
			end
			local olddata = readfile("goat/Profiles/bedwarsdata.txt")
			local newdata = game:HttpGet(url, true)
			if newdata ~= olddata then 
				rundata(game:GetService("HttpService"):JSONDecode(newdata), game:GetService("HttpService"):JSONDecode(olddata))
				olddata = newdata
				writefile("goat/Profiles/bedwarsdata.txt", newdata)
			else
